...
class Evolution_Admin_Insert {
	...
	public function media_buttons( $editor_id = 'content' ) {
		...
		if ( in_array( $pagenow, array('post.php', 'page.php', 'post-new.php', 'post-edit.php') ) ) {
            $output = '<a href="#TB_inline?width=4000&amp;inlineId=tzsc-choose-shortcode" class="thickbox button tzsc-thicbox" title="' . __( 'Insert Shortcode', 'evo-shortcodes' ) . '">' . __( 'Insert Shortcode', 'evo-shortcodes' ) . '</a>';
		}
		echo $output;
	}
			...
			<div id="tzsc-choose-shortcode" style="display: none;">
				<div id="tzsc-shortcode-wrap" class="wrap tzsc-shortcode-wrap">
					<div class="tzsc-shortcode-select">
                        <label for="tzsc-shortcode"><?php _e('Select the shortcode type', 'evo-shortcodes'); ?></label>
				...
				foreach( $evolution_shortcodes as $key => $shortcode ) {
					...
					if( array_key_exists('child_shortcode', $shortcode ) ) {
						...
						if( $clone_button['show'] ) {
                            $html .= '<tr><td colspan="2"><a href="#" class="tzsc-remove">' . __('Remove', 'evo-shortcodes') . '</a></td></tr>';
						}
						$html .= '</tbody></table>';
					}
				...
				<p class="submit">
                    <input type="button" id="tzsc-insert-shortcode" class="button-primary" value="<?php _e('Insert Shortcode', 'evo-shortcodes'); ?>" onclick="tzscInsertShortcode();" />
                    <a href="#" id="tzsc-cancel-shortcode-insert" class="button-secondary tzsc-cancel-shortcode-insert" onclick="tb_remove();"><?php _e('Cancel', 'evo-shortcodes'); ?></a>
				</p>
				</div>
			</div>
